...
History:
* 2021-01-24: reorganzied code for package and CLI interface
* 2013-01-11: renamed mhtifier.
* 2013-01-10: created mht2fs.py, and... done.

------------
Borrowed and modified to only unpack mht files.

Additional code to walk and recreate directory structure while unpacking mht files along the way

Also, for the love all that is holy...formatting the unwrapped comment lines. Always thought the 
79 char 'rule' was silly until I started working on a laptop full time. 
"""
    ...
    for (root, _, files) in os.walk("."):
        ...
        for f in files:
            ...
            if t and t.startswith("text/"):
                ...
                #??? WTF?
                m.set_payload(quopri.encodestring(open(p, "rt").read().encode("utf-8")).decode("ascii")) 
            else:
...
# Just do it.
# No make functions for reusable code, wait leave that for someone else. Oh, hello. I'm Someone Else.
def main():
    """Convert MHT file given as command line argument (or stdin?) to files and 
    ...
    parser.add_argument("d", metavar="DIR", help="directory to create to store parts in, or read them from.") 
